...
export default function CombinedChart({
  ...
  const { bars, maxValue, yAxisLabels } = useMemo(() => {
    ...
    const bars = data.map((point, index) => {
      ...
      const revenueHeight =
        (point.revenue / adjustedMax) *
        (height - CHART_PADDING_TOP - CHART_PADDING_BOTTOM);
      const chargesHeight =
        (point.charges / adjustedMax) *
        (height - CHART_PADDING_TOP - CHART_PADDING_BOTTOM);
      const profitHeight =
        (point.profit / adjustedMax) *
        (height - CHART_PADDING_TOP - CHART_PADDING_BOTTOM);
    ...
    <View style={styles.wrapper}>
        ...
        <View style={[styles.chartArea, { width: chartWidth, height }]}>
          ...
          {yAxisLabels.map((_, index) => {
            ...
            return (
              <View
                key={index}
                style={[styles.gridLine, { top: y, width: chartWidth }]}
              />
            );
          })}
      ...
      <View style={styles.legend}>
        <View style={styles.legendItem}>
          <View
            style={[styles.legendColor, { backgroundColor: BluePalette.merge }]}
          />
