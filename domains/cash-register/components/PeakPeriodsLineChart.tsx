...
import { formatAmountForDisplay, formatAmountMAD } from "@/utils/formatAmount";
...
export default function PeakPeriodsLineChart({
  ...
  const { pathData, areaPath, points, maxValue, yAxisLabels, xAxisLabels } =
    useMemo(() => {
      ...
      const plotWidth =
        chartWrapperWidth - CHART_PADDING_LEFT - CHART_PADDING_RIGHT;
      const plotHeight =
      ...
      const points = filteredHourlyData.map((data, index) => {
        const x =
          CHART_PADDING_LEFT +
          (index / Math.max(filteredHourlyData.length - 1, 1)) * plotWidth;
        const y =
          CHART_PADDING_TOP +
          plotHeight -
  ...
  const formatCurrency = (amount: number) => formatAmountForDisplay(amount, 0);

  const formatCurrencyFull = (amount: number) => formatAmountMAD(amount);
      ...
      <View style={styles.chartContainer}>
        ...
        <View style={[styles.chartWrapper, { width: chartWrapperWidth }]}>
          <Svg
            ...
            height={CHART_HEIGHT}
            viewBox={`0 0 ${chartWrapperWidth} ${CHART_HEIGHT}`}
          >
            <Defs>
              <LinearGradient
                id="areaGradient"
              ...
              >
                <Stop
                  offset="0%"
                  stopColor={BluePalette.surface}
                  stopOpacity="0.8"
                />
                <Stop
                  offset="100%"
                  stopColor={BluePalette.surfaceLight}
                  stopOpacity="0.1"
                />
              </LinearGradient>
            </Defs>
          ...
          <View style={styles.xAxisContainer}>
            {xAxisLabels.map((label, index) => {
              ...
              const labelLeft = Math.max(
                0,
                Math.min(relativeX - 25, availableWidth - 50),
              );
              return (
                <View
                  key={index}
